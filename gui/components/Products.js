...
const Products = ({ products }) => {
  ...
  return (
    <div className=" lg:container mx-auto mt-10 px-5 sm:px-0">
      <div className="flex justify-between items-center mb-4">
        <h1 className="font-bold text-xl">Recommend For You</h1>
        <div className="flex items-center gap-2 border-dashed border-b border-gray-300">
          More products
        ...
        {products
          .map((product) => {
            return (
              <SwiperSlide
                key={product._id}
                className="group border rounded-lg cursor-pointer"
              >
                ...
                <i className="absolute top-5 right-5 animate-[wiggle_1s_ease-in-out_infinite] text-2xl p-3 opacity-50 border border-black rounded-full bg-white">
                  <BsCart2 />
                </i>
                <div className="flex items-center justify-between py-5 px-2">
                  <h1 className="text-xl sm:text-xs">{product.name}</h1>
